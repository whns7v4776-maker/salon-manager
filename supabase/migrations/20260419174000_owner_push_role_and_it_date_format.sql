...
begin
  ...
  v_role_label := nullif(trim(coalesce(new.required_role, '')), '');
  ...
  if tg_op = 'INSERT' then
    if new.origin = 'frontend' and new.status = 'pending' then
      ...
      v_body :=
        ...
        case
          when v_role_label is not null then
            '. Mestiere: ' || v_role_label
          else ''
        end ||
        case
      ...
      v_payload := jsonb_build_object(
        ...
        'roleName', coalesce(v_role_label, ''),
  ...
  if tg_op = 'UPDATE' and new.status is distinct from old.status then
    ...
    v_title := case v_status_label
      ...
      else 'Richiesta annullata dal cliente'
    end;
    v_body :=
      v_status_label ||
      case
