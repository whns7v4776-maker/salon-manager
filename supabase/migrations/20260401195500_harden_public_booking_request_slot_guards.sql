create index if not exists booking_requests_workspace_active_slot_idx
on public.booking_requests (workspace_id, appointment_date, appointment_time)
where status in ('pending', 'accepted');

create index if not exists appointments_workspace_slot_idx
...
begin
  ...
  if exists (
    ...
    raise exception 'slot_already_requested';
  end if;
  ...
  if exists (
    ...
    where booking_request.workspace_id = v_workspace.id
      ...
      and booking_request.appointment_time = p_appointment_time
