...
begin
  ...
  from (
    ...
    where (
      ...
      or lower(trim(coalesce(client_item ->> 'nome', ''))) = lower(v_customer_full_name)
    )
    limit 1
  ) as matched_client;
  ...
  if v_max_future_appointments is not null and v_max_future_appointments >= 0 then
    with appointment_slots as (
      select distinct
        ...
        lower(trim(coalesce(appointment.service_name, ''))) as service_name,
        lower(trim(coalesce(appointment.customer_name, ''))) as customer_name
    ...
    request_slots as (
      select distinct
        ...
        lower(trim(coalesce(booking_request.requested_service_name, ''))) as service_name,
        lower(trim(concat_ws(' ', coalesce(booking_request.customer_name, ''), coalesce(booking_request.customer_surname, '')))) as customer_name
    ...
    from (
      ...
      union
      select request_slots.*
      from request_slots
      where not exists (
        select 1
        from appointment_slots
        where appointment_slots.appointment_date = request_slots.appointment_date
          and appointment_slots.appointment_time = request_slots.appointment_time
          and appointment_slots.service_name = request_slots.service_name
          and appointment_slots.customer_name = request_slots.customer_name
      )
    ) as unique_future_bookings;
