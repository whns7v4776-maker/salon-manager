...
drop function if exists public.is_public_booking_slot_available(uuid, date, time, integer);
  ...
  with requested_range as (
    select
      ...
      p_appointment_date::timestamp + p_appointment_time
        + make_interval(mins => greatest(coalesce(p_requested_duration_minutes, 60), 1)) as end_at,
      nullif(trim(coalesce(p_requested_operator_id, '')), '') as requested_operator_id,
      nullif(lower(trim(coalesce(p_requested_operator_name, ''))), '') as requested_operator_name
  )
  select not exists (
    select 1
    ...
    where (p_appointment_date::timestamp + appointment.appointment_time) < rr.end_at
      ...
      and (
        rr.requested_operator_id is null
        or (
          nullif(trim(coalesce(appointment.operator_id, '')), '') is null
          and nullif(lower(trim(coalesce(appointment.operator_name, ''))), '') is null
        )
        or nullif(trim(coalesce(appointment.operator_id, '')), '') = rr.requested_operator_id
        or (
          rr.requested_operator_name is not null
          and nullif(lower(trim(coalesce(appointment.operator_name, ''))), '') = rr.requested_operator_name
        )
      )
  )
  and not exists (
    ...
    where (p_appointment_date::timestamp + booking_request.appointment_time) < rr.end_at
      ...
      and (
        rr.requested_operator_id is null
        or (
          nullif(trim(coalesce(booking_request.requested_operator_id, '')), '') is null
          and nullif(lower(trim(coalesce(booking_request.requested_operator_name, ''))), '') is null
        )
        or nullif(trim(coalesce(booking_request.requested_operator_id, '')), '') = rr.requested_operator_id
        or (
          rr.requested_operator_name is not null
          and nullif(lower(trim(coalesce(booking_request.requested_operator_name, ''))), '') = rr.requested_operator_name
        )
      )
  );
$$;
...
grant execute on function public.is_public_booking_slot_available(uuid, date, time, integer, text, text) to anon;
grant execute on function public.is_public_booking_slot_available(uuid, date, time, integer, text, text) to authenticated;
...
drop function if exists public.create_owner_appointment(
  text,
  text,
  citext,
  text,
